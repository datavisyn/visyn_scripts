...
class Generator extends Base {
  constructor(args, options) {
    super(args, options);
    this.productName = RepoUtils.toBaseName(this.options.product);
    this.cwd = RepoUtils.toCWD(this.productName);
    this.cloneSSH = this.options.ssh;
  }
  ...
  _removeUnnecessaryProductFiles() {
    ...
    fs.rm(`${this.cwd}/.git`, { recursive: true });
   ...
   */
  _getProduct() {
    return WorkspaceUtils.cloneRepo(this.productName, this.options.branch || 'main', null, '.', this.cwd, this.cloneSSH).then(() => {
  ...
  _downloadDataFiles() {
    ...
    return GeneratorUtils.mkdir(`${this.cwd}/_data`).then(() => Promise.all(data.map((d) => this._downloadDataFile(d, `${this.cwd}/_data`))));
  }
  ...
  _downloadBackupFiles() {
    ...
    return GeneratorUtils.mkdir(`${this.cwd}/_backup`)
      ...
      .then(
        this._ifExecutable.bind(
          this,
          'docker',
          () => SpawnUtils.spawnOrAbort('./docker-backup', 'restore', this.cwd, true),
          'please execute: "./docker-backup restore" manually',
        ),
      );
  }
  ...
  _buildDockerCompose() {
    ...
    if (isNotEmpty && !this.options.skip.includes('build')) {
      return this._ifExecutable(
        'docker',
        () => SpawnUtils.spawnOrAbort('docker', 'compose build', this.cwd, true),
        ' please run "docker compose build" manually"',
      );
    }
