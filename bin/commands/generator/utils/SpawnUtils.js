...
const { spawnPr } = require('child-process-promise');
...
module.exports = class SpawnUtils {
   ...
   */
  static abort(msg) {
    return Promise.reject(msg || 'Step Failed: Aborting');
  }
  ...
  static async spawnPromise(command, argline, cwd) {
    ...
    return spawnPr(command, Array.isArray(argline) ? argline : argline.split(' '), options).catch((e) => {
